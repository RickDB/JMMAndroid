...
namespace JMMAndroid
{
	public class Utils
	{
        ...
        {
            if (DateTime.Now > MainActivity.NextVersionCheck)
            {
                MainActivity.NextVersionCheck = DateTime.Now.AddMinutes(30);
        ...
        private static void GetLatestVersionThread(object obj)
        {
            ...
            if (!string.IsNullOrEmpty(xml))
            {
                ...
                if (m.Success)
                {
                    MainActivity.LastestVersion = m.Groups["Major"] + "." + m.Groups["Minor"] + "." + m.Groups["Build"] + "." + m.Groups["Revision"];
                    return;
                }

            }
            MainActivity.LastestVersion = string.Empty;
        }
